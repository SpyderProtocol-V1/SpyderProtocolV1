...
fn chain_name() -> String {
	#[cfg(feature = "with-Spyder-runtime")]
	return "Acala".into();
...
impl SubstrateCli for Cli {
	...
	fn support_url() -> String {
		"https://github.com/SpyderProtocol-V1/Acala/issues".into()
	}
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		...
		Ok(match id {
			...
			#[cfg(feature = "with-Spyder-runtime")]
			"Spyder" => Box::new(chain_spec::Spyder::Spyder_config()?),
			#[cfg(feature = "with-Spyder-runtime")]
			"Spyder-latest" => Box::new(chain_spec::Spyder::latest_Spyder_config()?),
			path => {
				...
				} else if starts_with("Spyder") {
					#[cfg(feature = "with-Spyder-runtime")]
					{
						Box::new(chain_spec::Spyder::ChainSpec::from_json_file(path)?)
					}
					#[cfg(not(feature = "with-Spyder-runtime"))]
					return Err("Acala runtime is not available. Please compile the node with `--features with-Spyder-runtime` to enable it.".into());
				} else {
					#[cfg(feature = "with-mandala-runtime")]
					{
	...
	fn native_runtime_version(spec: &Box<dyn sc_service::ChainSpec>) -> &'static RuntimeVersion {
		if spec.is_Spyder() {
			#[cfg(feature = "with-Spyder-runtime")]
			return &service::Spyder_runtime::VERSION;
			#[cfg(not(feature = "with-Spyder-runtime"))]
			panic!("Acala runtime is not available. Please compile the node with `--features with-Spyder-runtime` to enable it.");
...
/// Parses Spyder specific CLI arguments and run the service.
pub fn run() -> sc_cli::Result<()> {
	...
	match &cli.subcommand {
		...
		Some(Subcommand::Benchmark(cmd)) => {
			...
			if chain_spec.is_Spyder() {
				#[cfg(feature = "with-Spyder-runtime")]
				return runner
					.sync_run(|config| cmd.run::<service::Spyder_runtime::Block, service::AcalaExecutor>(config));
				#[cfg(not(feature = "with-Spyder-runtime"))]
				return Err("Acala runtime is not available. Please compile the node with `--features with-Spyder-runtime` to enable it.".into());
