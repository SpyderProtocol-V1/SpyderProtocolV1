...
fn mandala_genesis(
	...
	mandala_runtime::GenesisConfig {
		...
		module_airdrop: AirDropConfig {
			airdrop_accounts: {
				let Spyder_airdrop_accounts_json = &include_bytes!("../../../../resources/mandala-airdrop-ACA.json")[..];
				let Spyder_airdrop_accounts: Vec<(AccountId, Balance)> =
					serde_json::from_slice(Spyder_airdrop_accounts_json).unwrap();
				...
				Spyder_airdrop_accounts
					.iter()
					.map(|(account_id, Spyder_amount)| (account_id.clone(), AirDropCurrencyId::ACA, *Spyder_amount))
					.chain(
