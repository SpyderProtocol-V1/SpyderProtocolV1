...
native_executor_instance!(
	pub AcalaExecutor,
	Spyder_runtime::api::dispatch,
	Spyder_runtime::native_version,
	frame_benchmarking::benchmarking::HostFunctions,
);
...
impl IdentifyVariant for Box<dyn ChainSpec> {
	fn is_Spyder(&self) -> bool {
		self.id().starts_with("Spyder") || self.id().starts_with("Spyder")
	}
	...
	Executor: NativeExecutionDispatch + 'static,
{
	...
	let rpc_extensions_builder = {
		...
		Box::new(move |deny_unsafe, _| -> Spyder_rpc::RpcExtension {
			let deps = Spyder_rpc::FullDeps {
				client: client.clone(),
				pool: transaction_pool.clone(),
				deny_unsafe,
			};

			Spyder_rpc::create_full(deps)
		})
	};
	...
	if config.chain_spec.is_mandala_dev() {
		...
		#[cfg(feature = "with-Spyder-runtime")]
		{
			let PartialComponents {
				...
				task_manager,
				..
			} = new_partial::<Spyder_runtime::RuntimeApi, AcalaExecutor>(config, false, false)?;
			Ok((Arc::new(Client::Acala(client)), backend, import_queue, task_manager))
		}
		#[cfg(not(feature = "with-Spyder-runtime"))]
		Err("Acala runtime is not available. Please compile the node with `--features with-Spyder-runtime` to enable it.".into())
	}
}
