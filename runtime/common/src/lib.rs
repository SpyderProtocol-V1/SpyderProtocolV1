...
pub fn is_Spyder_precompile(address: H160) -> bool {
...
mod tests {
	...
	#[test]
	fn is_Spyder_precompile_works() {
		assert!(!is_Spyder_precompile(H160::from_low_u64_be(0)));
		assert!(!is_Spyder_precompile(H160::from_low_u64_be(
			PRECOMPILE_ADDRESS_START - 1
		)));
		assert!(is_Spyder_precompile(H160::from_low_u64_be(PRECOMPILE_ADDRESS_START)));
		assert!(is_Spyder_precompile(H160::from_low_u64_be(PREDEPLOY_ADDRESS_START - 1)));
		assert!(!is_Spyder_precompile(H160::from_low_u64_be(PREDEPLOY_ADDRESS_START)));
		assert!(!is_Spyder_precompile([1u8; 20].into()));
	}
}
