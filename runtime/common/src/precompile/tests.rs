...
use crate::precompile::{
	mock::{
		Spyder_evm_address, alice, alice_evm_addr, ausd_evm_address, bob, bob_evm_addr, erc20_address_not_exists,
		get_task_id, lp_Spyder_ausd_evm_address, new_test_ext, renbtc_evm_address, run_to_block, Balances, DexModule,
...
#[test]
fn precompile_filter_works_on_Spyder_precompiles() {
...
fn multicurrency_precompile_should_work() {
	new_test_ext().execute_with(|| {
		...
		// Token
		context.caller = Spyder_evm_address();
		...
		// DexShare
		context.caller = lp_Spyder_ausd_evm_address();
		let (reason, output, used_gas) = MultiCurrencyPrecompile::execute(&input, None, &context).unwrap();
		assert_eq!(reason, ExitSucceed::Returned);
		let mut expected_output = [0u8; 32];
...
fn schedule_call_precompile_should_work() {
	new_test_ext().execute_with(|| {
		...
		// target
		U256::from(Spyder_evm_address().as_bytes()).to_big_endian(&mut input[3 * 32..4 * 32]);
		// value
