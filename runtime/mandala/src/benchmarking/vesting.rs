...
use super::utils::{lookup_of_account, set_Spyder_balance};
use crate::{
...
runtime_benchmarks! {
	...
	vested_transfer {
		...
		set_Spyder_balance(&from, schedule.total_amount().unwrap() + dollar(ACA));
	...
	claim {
		...
		let from: AccountId = AcalaTreasuryPalletId::get().into_account();
		// extra 1 dollar to pay fees
		set_Spyder_balance(&from, schedule.total_amount().unwrap() * i as u128 + dollar(ACA));
	...
	update_vesting_schedules {
		...
		let to: AccountId = account("to", 0, SEED);
		set_Spyder_balance(&to, schedule.total_amount().unwrap() * i as u128);
		let to_lookup = lookup_of_account(to.clone());
